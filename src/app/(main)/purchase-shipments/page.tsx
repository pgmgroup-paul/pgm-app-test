...
async function loadShipments({ search, status = "all", sort = "etd_asc" }: LoadOptions): Promise<ShipmentRow[]> {
  ...
  if (error) {
    console.error("Error loading shipments for /purchase-shipments", error);
    return [];
  }
...
export default async function PurchaseShipmentsPage({
  searchParams,
}: {
    ...
    <div className="space-y-4 p-6">
      <div className="flex items-center justify-between">
        <div className="space-y-1">
          <h1 className="font-semibold text-2xl tracking-tight">Purchase shipments</h1>
          <p className="text-muted-foreground text-sm">Plan and track inbound shipments that group containers and POs.</p>
        </div>
        <Link
          href="/purchase-shipments/new"
