...
export default async function ShipmentContainerPage({
  ...
  const { data: openPos, error: poError } = await serverSupabase
    ...
    .select(
      `po_number,
       status,
       purchase_order_lines(id, description)`,
    )
    .eq("status", "open")
  ...
  // Build friendly labels for PO dropdown: PO_NUMBER — NAME1, NAME2, NAME3, NAME4
  const poLabelMap = new Map<string, string>();

  // Helper: normalize product names for display (Title Case, but preserve acronyms/numeric codes)
  function formatProductName(name: string): string {
    const trimmed = name.trim();
    if (!trimmed) return "";

    const acronymWords = new Set(["LED", "USB", "RGB", "HDMI", "TV", "UV"]);
    ...
    return trimmed
      ...
      .map((word) => {
        ...
        // If word contains any digit, keep as-is (e.g. 20QT, 500ML)
        if (/\d/.test(word)) return word;

        const lettersOnly = word.replace(/[^A-Za-z]+/g, "");
        // Preserve known acronyms exactly (LED, USB, RGB, etc.)
        if (lettersOnly && acronymWords.has(lettersOnly)) {
          return word;
        }
  ...
  // Simple keyword extraction heuristic for product names
  ...
  function buildShortName(rawName: string): string | null {
    const normalized = formatProductName(rawName);
    const cleaned = normalized.trim();
    if (!cleaned) return null;
    const words = cleaned.split(/\s+/);

    // Build candidate keywords while preserving original order
    const primary: string[] = []; // non-stop, non-color words
    const secondary: string[] = []; // color words

    for (const w of words) {
      const lower = w.toLowerCase();
      if (stopWords.has(lower)) continue;
      if (colorWords.has(lower)) {
        secondary.push(w);
      } else {
        primary.push(w);
      }
    ...
    // Fallback: first 2 words from original name
  ...
  for (const po of openPos ?? []) {
    ...
    const lines = ((po as any).purchase_order_lines as any[]) || [];

    const productNames: string[] = [];

    for (const line of lines) {
      const rawName = ((line as any).description as string) || "";
      if (!rawName) continue;

      const shortName = buildShortName(rawName);
      if (!shortName) continue;

      productNames.push(shortName);
      if (productNames.length >= 4) break; // Limit to 4 products
    }

    const label =
      productNames.length > 0 ? `${poNumber} — ${productNames.join(", ")}` : poNumber;

    poLabelMap.set(poNumber, label);
  }
                              ...
                              <form
                                action={async (formData: FormData) => {
                                  ...
                                  if (
                                    ...
                                    redirect(`/sales-shipments/${shipmentIdFromForm}/containers/${containerIdFromForm}`);
                                  }
              ...
              <form
                action={async (formData: FormData) => {
                  ...
                  if (!containerIdFromForm) {
                    redirect(`/sales-shipments/${shipmentId}/edit`);
                  }
                  ...
                  if (updError) {
                    ...
                    redirect(`/sales-shipments/${shipmentId}/containers/${containerIdFromForm}`);
                  }

                  // On success, stay on the same page and surface a success flag via search params.
                  redirect(`/sales-shipments/${shipmentId}/containers/${containerIdFromForm}?success=container-updated`);
                }}
                ...
                <form
                  action={async (formData: FormData) => {
                    ...
                    if (!shipmentIdFromForm || !containerIdFromForm || !poLineId || !qtyRaw) {
                      redirect(
                        `/sales-shipments/${shipmentIdFromForm}/containers/${containerIdFromForm}?po_number=${poNumberFromForm}&error=missing-fields`,
                      );
                    }
                    ...
                    if (!Number.isFinite(qty) || qty <= 0) {
                      redirect(
                        `/sales-shipments/${shipmentIdFromForm}/containers/${containerIdFromForm}?po_number=${poNumberFromForm}&error=bad-qty`,
                      );
                    }
                    ...
                    if (lineError || !line) {
                      ...
                      redirect(
                        `/sales-shipments/${shipmentIdFromForm}/containers/${containerIdFromForm}?po_number=${poNumberFromForm}&error=load-line`,
                      );
                    }
                    ...
                    if (maxUnitsFromOrder > 0 && totalIfAllocated > maxUnitsFromOrder) {
                      redirect(
                        `/sales-shipments/${shipmentIdFromForm}/containers/${containerIdFromForm}?po_number=${poNumberFromForm}&error=over-order`,
                      );
                    }

                    if (maxUnitsFromOrder > 0 && totalIncludingReceived > maxUnitsFromOrder) {
                      redirect(
                        `/sales-shipments/${shipmentIdFromForm}/containers/${containerIdFromForm}?po_number=${poNumberFromForm}&error=over-order-inc-received`,
                      );
                    }
                    ...
                    if (insError) {
                      ...
                      redirect(
                        `/sales-shipments/${shipmentIdFromForm}/containers/${containerIdFromForm}?po_number=${poNumberFromForm}&error=insert-item`,
                      );
                    }
                    ...
                    redirect(
                      `/sales-shipments/${shipmentIdFromForm}/containers/${containerIdFromForm}?po_number=${poNumberFromForm}`,
                    );
                  }}
                >
                  <input type="hidden" name="shipment_id" value={shipment.id as string} />
                        ...
                        {poLines.map((line: any) => {
                          ...
                          return (
                            <tr key={line.id} className="border-b last:border-none">
                              ...
                              <td className="px-2 py-1 text-[11px]">{formatProductName(line.description as string)}</td>
                              ...
                              <td className="px-2 py-1 text-right text-[11px]">
                                <Link
                                  href={`/sales-shipments/demand?sku=${encodeURIComponent(
                                    (line.sku as string) || "",
