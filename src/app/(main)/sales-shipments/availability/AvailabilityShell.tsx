...
export function AvailabilityShell({
  ...
  initialSku,
  initialVar,
}: AvailabilityShellProps) {
  const [state, formAction] = useActionState<AvailabilityState, FormData>(loadAvailability, initialState);

  return (
    <div className="max-w-4xl space-y-4 p-6">
      <div className="space-y-1">
        <h1 className="font-semibold text-2xl tracking-tight">Inventory Availability</h1>
            ...
            <div className="space-y-1 rounded-md bg-muted px-3 py-2 text-[11px]">
              ...
              <div className="grid grid-cols-1 gap-2 sm:grid-cols-5">
                <div>
                  ...
                  <div className="font-mono text-sm">{state.incomingUnits ?? 0} units</div>
                </div>
                <div>
                ...
                <div>
                  <div className="text-muted-foreground">Available incl. incoming</div>
                  <div
                    className={`font-mono text-sm${
                      (state.availableInclIncoming ?? 0) < 0 ? " font-semibold text-destructive" : ""
                    }`}
                  >
                    {state.availableInclIncoming ?? 0} units
                  </div>
                </div>
              </div>
            </div>
                  ...
                  <table className="w-full text-left text-[11px]">
                    ...
                    <tbody>
                      {state.orders.map((o, idx) => (
                        <tr
                          key={`${o.so_number}-${o.ship_date || "no-date"}-${idx}`}
